...
public class WebService {
    ...
    public void registerByFB(RequestParams params,String url,final Context context,String email){
        // Show Progress Dialog
        //prgDialog.show();
        // Make RESTful webservice call using AsyncHttpClient object
        AsyncHttpClient client = new AsyncHttpClient();
        ...
        client.post(url,params ,new JsonHttpResponseHandler() {
            ...
            public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
                ...
                for (int i = 0;i <headers.length;i++){
                    Log.d("headers", String.valueOf(headers[i]));
                }
                try {
                    Log.i(WebService.this.toString(),"Server response  : ----> "+response);
                    ...
                    if(obj.getBoolean("status")){
                        ...
                        context.startActivity(startActivity);
                        Toast.makeText(context, "Bienvenu", Toast.LENGTH_LONG).show();
                    }
                    // Else display error message
                    else{
                        Log.i(WebService.this.toString(),"Error Occured : "+obj.getString("response"));
